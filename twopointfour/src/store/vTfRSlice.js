...
export const vTfRAction = vTfRSlice.actions;

export const startvTfR = () => {
  return (dispatch, getState) => {
    function updateCountdown() {
      const state = getState();
      const lagTime = Date.now() - state.timer.expectedFunctionExecutionTime;
      dispatch(vTfRAction.updateSetTime());
      dispatch(vTfRAction.updateExecutionTime(state.timer.timerUpdateInterval));
      if (!state.timer.pause) {
        setTimeout(updateCountdown, Math.max(0, state.timer.timerUpdateInterval - lagTime));
      }
    }
    const state = getState();
    dispatch(vTfRAction.pauseTimer(false));
    dispatch(vTfRAction.initialExecutionTime());
