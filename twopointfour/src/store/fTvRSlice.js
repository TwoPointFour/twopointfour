...
export const fTvRAction = fTvRSlice.actions;

export const startfTvR = () => {
  return (dispatch, getState) => {
    function updateCountdown() {
      const state = getState();
      const lagTime = Date.now() - state.timer.expectedFunctionExecutionTime;
      dispatch(fTvRAction.updateSetTime());
      dispatch(fTvRAction.updateExecutionTime(state.timer.timerUpdateInterval));
      if (!state.timer.pause) {
        setTimeout(updateCountdown, Math.max(0, state.timer.timerUpdateInterval - lagTime));
      }
    }
    const state = getState();
    dispatch(fTvRAction.pauseTimer(false));
    dispatch(fTvRAction.initialExecutionTime());
